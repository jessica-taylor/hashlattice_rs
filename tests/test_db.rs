...
impl LatticeLibrary<EmptyMapping, MaxTupleMapping, EmptyMapping> for MaxTupleLatLibrary {
    fn check_elem(self: Arc<Self>, _key: &String, value: &Vec<usize>, _ctx: &dyn LatticeImmutContext<EmptyMapping, MaxTupleMapping, EmptyMapping>) -> Res<()> {
        if value.len() != self.0 {
            bail!("wrong length");
        }
        Ok(())
    }
    fn join(self: Arc<Self>, _key: &String, value1: &Vec<usize>, value2: &Vec<usize>, _ctx: &dyn LatticeMutContext<EmptyMapping, MaxTupleMapping, EmptyMapping>) -> Res<Vec<usize>> {
...
async fn test_db() {
    ...
    let store = Arc::new(LatStore::new(db, EmptyComputationLibrary, MaxTupleLatLibrary(3)));
